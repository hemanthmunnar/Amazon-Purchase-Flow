...
test('Amazon Shopping Flow', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Computers', exact: true }).click({ force: true });
  ...
  await expect(laptopResults.nth(1)).toBeVisible({ timeout: 20000 });
  ...
  await expect(page.locator('#sc-active-cart .sc-list-item')).toHaveCount(0, { timeout: 20000 });
  ...
  await expect(phoneLink).toBeVisible({ timeout: 20000 });
  ...
  await expect(cartLink).toContainText(/1|2/);
